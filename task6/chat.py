        ...
        try:
            ...
            # Realm-related
                ...
                message = ""
                    ...
                    # Check if sessionid exists in self.sessions
...
# IMPLEMENTATION FUNCTIONS
    ...
    def send_group_realm(self, sessionid, src_realm_addr, src_realm_port, realm_id, username_from, groupname, message):
        logging.debug(f"send_group_realm called with sessionid={sessionid}, src_realm_addr={src_realm_addr}, src_realm_port={src_realm_port}, realm_id={realm_id}, username_from={username_from}, groupname={groupname}, message={message}")

        if sessionid not in self.sessions:
            logging.error(f"Session ID {sessionid} not found in sessions")
            return {'status': 'ERROR', 'message': 'Session Tidak Ditemukan'}

        if realm_id not in self.realms_info:
            logging.error(f"Realm ID {realm_id} not found in realms_info")
            return {'status': 'ERROR', 'message': 'Realm belum ada'}

        if groupname not in self.groups:
            logging.error(f"Group name {groupname} not found in groups")
            return {'status': 'ERROR', 'message': 'Group tidak ditemukan'}

        group = self.groups[groupname]
        if username_from not in group['members']:
            logging.error(f"Username {username_from} not a member of group {groupname}")
            return {'status': 'ERROR', 'message': 'Bukan member group'}
